---
title: 'Tạo một chương trình quét đơn giản bằng Winform CLR'
date: 2021-08-07T15:32:14Z
lastmod: '2021-12-15'
tags: ['Winform', 'CLI/C++', 'Multi Threading']
draft: false
summary: 'Sử dụng mã hex để so sánh signature và lập trình đa luồng cho nó'
layout: PostSimple
bibliography: references-data.bib
---

## 1.Bước đầu tạo giao diện đơn giản

Mình dùng Visual Studio 2022:

sss

## Theme colors

Mình có một chương trình nhỏ kiểm tra xem mã hex đó có ở trong file hay không và không kiểm tra những file lớn hơn 50MB.

```js:MyForm.h
//-------------------------Thread--------------------------
	int n = 0;
	void test_thread(Object^ data) {
		...
		if (in.is_open()) {
			if (in.tellg() > 1000000) {
				...
				std::string flag = context.marshal_as<std::string>(textBox3->Text);
				...
				while (getline(in, line)) {
					for (int i = 0; i < line.size(); i++) {
						...
						sstream << std::hex << (line[i] - 0);
						result += sstream.str();
					}
				}
				if (result.find(flag) != string::npos) {
					System::String^ namee = gcnew String(path.c_str());
					textBox2->Text += namee + "----- Co " + "\r\n";
					textBox4->Text += namee +  "\r\n";
					n++;
					label4->Text = "";
					label4->Text += n ;
...
Luk 678 color attribute should be assigned an object with keys from 50, 100, 200 ... 900 and the corresponding color code values.
